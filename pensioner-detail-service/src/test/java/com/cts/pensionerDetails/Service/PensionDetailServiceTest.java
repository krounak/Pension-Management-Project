...
class PensionDetailServiceTest {
	...
	void testCorrectDetailsReturnedFromServiceWithCorrectAadharNumber()
		...
		PensionerDetails pensionerDetail = new PensionerDetails("Anand", DateUtil.parseDate("12-09-1956"),
				"BHMER12436", 27000, 10000, "self", new BankDetails("ICICI", 12345678, "private"));
		assertEquals(pds.getPensionerDetailByAadhaarNumber(token,aadhaarNumber).getPan(), pensionerDetail.getPan());
		assertEquals(pds.getPensionerDetailByAadhaarNumber(token,aadhaarNumber).getBank().getAccountNumber(), pensionerDetail.getBank().getAccountNumber());
	}
	...
	void testForIncorrectAadharNumber() {
		...
		NotFoundException exception = assertThrows(NotFoundException.class,
				() -> pds.getPensionerDetailByAadhaarNumber(token,aadhaarNumber));
